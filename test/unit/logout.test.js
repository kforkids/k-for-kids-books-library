...
// Regression: logging out used to leave the stats strip ("N read before")
// showing the PREVIOUS customer's data until the next loadBooks() server
// round-trip completed — a real, visible gap whenever that reload was slow.
// clearCustomerSession() now calls updateStats() itself so it hides
// immediately, without waiting on any network call.
function buildDocument() {
  const doc = fakeDocument();
  // Elements read by updateStats()/renderCustomerState() that the real
  // Index.html markup carries specific classes on.
...
describe('clearCustomerSession — logout hides customer-only UI synchronously', () => {
  ...
  beforeEach(() => {
    ...
    // exact state that produces "1 read before", matching the reported
    // scenario.
    app.run(`
  ...
  it('sanity check: the logged-in state shows the stats strip', () => {
